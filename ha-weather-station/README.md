...
* **Messwerte:** Temperatur und Luftfeuchte (SHT31) sowie Luftdruck (BMP280) werden über einen gemeinsamen I²C-Bus erfasst. Ein zweiter Fühler (AM2315) misst die Temperatur mit, geht aber in keine Rechnung ein und dient allein der Kreuzprüfung. Der Taupunkt wird nach der Magnus-Formel mit den Konstanten des Sensorherstellers aus Temperatur **und** Feuchte des SHT31 berechnet - beide Grössen aus demselben Chip, damit die relative Feuchte bei ihrer eigenen Messtemperatur ausgewertet wird.
...
Die Quelle ist über **Heater Source** umschaltbar. `Dew Point` (Default) hält den Sensor gezielt über dem Taupunkt und ist die energiesparendere Variante; `Ambient` bezieht sich auf die Umgebungstemperatur. Beide stammen aus dem SHT31 - fällt dieser Chip aus, liefert keine der beiden Quellen einen gültigen Sollwert und die Heizung bleibt über den Failsafe aus. Das ist die sichere Richtung, aber es heisst auch, dass `Ambient` kein Rückfall auf den zweiten Fühler ist.
...
* **Temperature Shed (`sensor.temperature_shed`):** Umgebungstemperatur, seit V3.2.0 vom **SHT31** (davor AM2315). Technisch eine Kopie des SHT31-Kanals, damit Name und `unique_id` und mit ihnen Verlauf und Langzeitstatistik erhalten bleiben. Während eines Wartungszyklus werden keine Werte publiziert, HA hält den letzten Stand.
...
* **Welcher der beiden Temperaturfühler recht hat, ist offen.** Über 169 Stunden gemessen trennt SHT31 und AM2315 ein konstanter Versatz von **+0.19 K**, unabhängig von der Temperaturrampe. Er enthält die Kalibrieroffsets zweier Exemplare zweier Hersteller; auflösen liesse ihn nur ein Referenzfühler. Seit V3.2.0 kommen Temperatur und Taupunkt beide aus dem SHT31, die Rechnung ist also in sich stimmig und die Spanne Temperatur minus Taupunkt trägt den Versatz nicht mehr - die Wahl fiel nach Datenblatt auf den SHT31 (±0.2 K gegen max. ±1 K), nicht nach Messung.
* **Der Verlauf von `sensor.temperature_shed` hat am 05.09.2026 einen Sprung von rund +0.19 K.** Das ist der Preis dafür, die Entität samt Historie zu behalten, statt eine neue anzulegen: Vor diesem Datum stammt die Reihe vom AM2315, danach vom SHT31. Vergleiche über dieses Datum hinweg sind um diesen Betrag verschoben.
* **Einmal pro Woche steht die Temperatur rund 17 Minuten still.** Während des SHT-Wartungszyklus (Heizen plus Erholung) publiziert der Chip nichts, und da die angezeigte Temperatur von ihm kommt, hält HA so lange den letzten Wert. Für Feuchte und Taupunkt galt das schon vorher.
