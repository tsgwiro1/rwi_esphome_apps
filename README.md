...
| :--- | :--- | :--- |
| [**`wp-fp1-smartblock`**](./wp-fp1-smartblock) | Intelligente, PV-optimierte Steuerung für die Heizkreispumpe (FP1) auf Basis eines XIAO ESP32-C6. Beinhaltet eine ausfallsichere Logik (NC-Verdrahtung) mit 7-stufiger Prioritäten-Kaskade, Sensor-Watchdog, lokale LDR-Displaysteuerung (ST7735) und einen DS18B20-Überhitzungsschutz. | v1.1.0 (Aktiv) |
| [**`ha-smartrelais`**](./ha-smartrelais) | Zeitgesteuertes SSR-Relais (ESP32-C6) nach dem Treppenlicht-Prinzip: Wandschalter startet eine einstellbare Einschaltdauer. Mit DS18B20-Übertemperaturschutz inkl. Wiedereinschaltsperre, Status-LED, Tageslimit und Betriebsstundenzählern. | v1.1.0 (Aktiv) |
...
| [**`ha-weather-station`**](./ha-weather-station) | ESP32-Wetterstation mit Temperatur, Luftfeuchte, Luftdruck und Taupunkt (SHT31 und BMP280, dazu ein AM2315 als Kontrollfühler). Frequenzbasierte Regenerkennung mit selbstlernender Trockenfrequenz, Hysterese, PID-geregelte Sensorheizung über Taupunkt sowie ein zeitgesteuerter Wartungszyklus für den SHT31-Heizer. | v3.2.1 (Aktiv) |
| [**`ha-frontroom-info-display`**](./ha-frontroom-info-display) | Wandmontiertes 2.8"-Touchdisplay (ESP32-2432S028R) mit sechs Seiten: Übersicht, Wallbox-Detail, Ladeplan-Eingabe, Hausbatterie mit 24-h-Graph, animiertes Energieflussbild und Wettervorhersage. Steuert den evcc-Lademodus und den SoC-Ladeplan direkt per REST, zusätzlich über zwei Hardware-Taster mit Status-LEDs und zwei Template-Schalter in HA. | v1.10.0 (Aktiv) |
...
| [**`wp-speicher-monitor`**](./wp-speicher-monitor) | ESP32-Schichtungsanzeige für den Wärmespeicher. Vier fest adressierte DS18B20 über die Speicherhöhe liefern das Temperaturprofil, daraus wird die Höhe der Sprungschicht und die grösste Nachbardifferenz abgeleitet. Ein 2.4"-ILI9341-Display am `mipi_spi`-Treiber zeigt die Schichten als Farbbalken mit Markierung der Schichtgrenze, dazu Heizraumklima per DHT; die Beleuchtung schaltet ein LDR mit Hysterese. Ein Fühler-Watchdog verwirft Werte, die seit über 300 s nicht mehr nachgeführt wurden. | v1.2.0 (Aktiv) |
| [**`wp-solar-monitor`**](./wp-solar-monitor) | ESP32-Wärmemengenzähler für den Solarkreis. Aus Vorlauf-, Rücklauftemperatur (DS18B20) und dem Kontakt der Solarpumpe wird alle 5 s eine Energie-Zeitscheibe gebildet und zu Tagesenergie und Momentanleistung verrechnet; Durchfluss, Dichte und Wärmekapazität sind aus Home Assistant einstellbar. Ein 280x240-ST7789V-Display am `mipi_spi`-Treiber zeigt das Anlagenschema mit Temperaturen, Leistung, Tagesertrag und Pumpenlaufzeit, die Beleuchtung schaltet ein LDR mit Hysterese. Ein Fühler-Watchdog hält die Zählung an, sobald ein DS18B20 seit über 60 s keinen Wert mehr liefert. | v1.1.0 (Aktiv) |
...
| [**`ha-mini-display`**](./ha-mini-display) | TTGO T-Display (ESP32, 1.14"-ST7789V) als Statusmonitor ohne eigene Logik. Blättert im 5-Sekunden-Takt durch sechs Seiten – Uhrzeit, Fahrzeug, Wallbox, PV-Leistung, Hausbatterie und Wetter – und speist sie aus fünfzehn Home-Assistant-Entitäten. Alle Symbole sind Glyphen aus der Material-Design-Icons-Schrift, das Gerät braucht keine Bilddateien. Die Fahrzeugseite unterscheidet Grigio vom Gastfahrzeug, und ohne angestecktes Fahrzeug fallen Fahrzeug- und Wallbox-Seite aus der Rotation; die Wetterseite deckt alle fünfzehn HA-Zustände mit Symbol, Text und Farbe ab. Zwei Taster melden `press` und `long_press` als Ereignisse. | v1.1.0 (Aktiv) |
